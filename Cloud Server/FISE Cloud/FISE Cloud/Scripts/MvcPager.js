...
Webdiyer.MvcPager.prototype = {
    ...
    init: function () {
        ...
        this.pageSizeBox = wrapper.data("pagesizebox");
        this.pageSizeName = wrapper.data("pagesizeparameter");
        this.currentPageSize = wrapper.data("currentpagesize") || 10;
        ...
        if (this.isAjaxPager) {
            ...
            var hashIndex = this.__getHashValue(this.pageIndexName);
            var hashPSize = this.__getHashValue(this.pageSizeName);
    ...
    __bindPageSizeBox: function () {
        var context = this;
        if (context.isAjaxPager) {
            if ($.trim(context.pageSizeBox) !== "") {
                if ($(context.updateTarget).find(context.pageSizeBox).length > 0) {
                    //page index box is inside of update target area
                    //check if page index box is dropdownlist and fill it with page indices
                    if ($(context.pageSizeBox).is('input:text')) {
                        $(context.pageSizeBox).val(context.currentPageSize);
                        $(context.updateTarget).on("keydown", context.pageSizeBox, function () {
                            context.__validateInput(event);
                        });
                    } else {
                        var se = $(this.pageSizeBox).val(this.currentPageSize);
                        $(context.updateTarget).on("change", context.pageSizeBox, function () {
                            context.currentPageSize = $(context.pageSizeBox).val();
                            context.allowReload = true;
                            context.searchCriteria = $(context.dataFormId).serializeArray();
                            context.__setPageIndex(context.pageIndexName, -1);
                            context.goToPage(1);
    ...
    __bindSizeBoxEvents: function () {
        var context = this;
        if ($(context.pageSizeBox).is('input:text')) {
            $(context.pageSizeBox).val(context.currentPageSize);
            $(context.pageSizeBox).keydown(function () {
                context.__validateInput(event);
            });
        }
        else {
            var se = $(this.pageSizeBox).val(this.currentPageSize);
            $(context.pageSizeBox).change(function () {
                context.currentPageSize = $(context.pageSizeBox).val();
                context.allowReload = true;
                context.searchCriteria = $(context.dataFormId).serializeArray();
                context.__setPageIndex(context.pageIndexName, -1);
    ...
    __initHashChange: function (initialPageIndex) {
        ...
        (docMode === undefined || docMode > 7)) //IE compatable mode
        {
            $(window).bind("hashchange", function () {
                var pageIndex = context.__getHashValue(context.pageIndexName);
                if (pageIndex === 0)
    ...
    __ajax: function (index, options) {
        ...
        if (typeof Webdiyer.__ajaxPages[context.pageIndexName] === "undefined" || Webdiyer.__ajaxPages[context.pageIndexName] !== index || context.allowReload) { //prevent duplicate ajax requests
            ...
            $.extend(options, {
                url: this.urlFormat.replace("__" + context.pageIndexName + "__", index).replace("__" + context.pageSizeName + "__", context.currentPageSize),
...
(function ($) {
    $.fn.initMvcPagers = function () {
        ...
        return this.each(function () {
            ...
            if (isAjaxPager) {
                var updateTarget = wrapper.data("ajax-update");
                if ($.inArray(updateTarget, Targets)<0)
                {
