...
namespace FISE_Browser.TWebClients
{
    public partial class TWebClient: ITWebClient
    {
        ...
        private static string _apiUserName = ConfigurationManager.AppSettings["ApiUserName"];
        private static string _apiUserKey = ConfigurationManager.AppSettings["ApiUserKey"];
        ...
        private List<string> XMLAPIs = new List<string> { "getbookscatlog", "syncuserbrowsingtime", "syncbookdata", "syncuserdata", "getuserdata" };
        ...
        public TWebClient()
        {
            ...
            var token = String.Format("{0}:{1}", _apiUserKey, _apiUserName);
            var tokenBase64 = Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(token));
            _webClient.Headers.Add("token", tokenBase64);
        }
        public void Dispose()
        {
 	        this.Dispose();
        ...
        public T UploadData<T>(string resourceName, object postData)
        {
            try
            {
                ...
                if (postData != null && !String.IsNullOrEmpty(resourceName.Trim()))
                {
                    ...
                    if (this.XMLAPIs.Contains(resourceName))
                    {
                        XmlSerializer MySerializer = new XmlSerializer(typeof(T));
                        s = s.Replace("\\\"", "\"").Trim('"');
                        t = (T)MySerializer.Deserialize(new StreamReader(new MemoryStream(Encoding.UTF8.GetBytes(s))));
                    }
                    else
                    {
